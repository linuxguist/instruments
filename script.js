...
if( letterVariable == 0 || letterVariable == 1 || letterVariable == 2 || letterVariable == 3 || letterVariable == 4 || letterVariable == 5 || letterVariable == 6 || letterVariable == 7 || letterVariable == 8 || letterVariable == 9 ) { 
  // code here    
  // console.log("eventkey received is : "+letterVariable);
  var listvalue = document.getElementById('instruments').value;
  if (listvalue != "") { 
    if (letterVariable != "0") {
  var newletterVariable = "id_"+"0"+letterVariable+"_"+listvalue.replace(/ /g,"_")+"_mp3";
} else {
  var newletterVariable = "id_"+"10"+"_"+listvalue.replace(/ /g,"_")+"_mp3";
}
// console.log("newletterVariable is : "+newletterVariable);
    var mp3Source = document.getElementById(newletterVariable);
    // console.log(mp3Source);
    // console.log(mp3Source.currentSrc);
    // mp3Source.src = mp3Source.currentSrc;
    // console.log(mp3Source.src);
    audioPlayer = document.getElementById(newletterVariable);
  ...
  alert('Please select an instrument first !');
    ...
    // console.log("eventkey not received is : "+letterVariable);
  }
}
...
letterOnScreen = document.getElementById('big-char'); // get the element properties
  ...
  letterOnScreen.onkeypress = function(event){ // Do stuff when you press any key in the document
    ...
    if ( // if use pressed on any of these characters which are not numbers or letters
      ...
      setLetterSound(event.key); // Set the generic sound in the resources of the audio tag
      runAudioPlayer(); // Play sound
      // generateNewElement(); // Generate new DOM element to restart the animation
      // changeEventHandler(event);
    } else {
...
$(document).ready(function(){
    document.getElementById("instruments").focus();
});